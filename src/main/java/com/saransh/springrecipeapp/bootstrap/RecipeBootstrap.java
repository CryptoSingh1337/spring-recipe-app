...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private List<Recipe> getRecipes() {
        ...
        UnitOfMeasure eachUom = (UnitOfMeasure) getOptional("Each", 1);
        UnitOfMeasure dashUom = (UnitOfMeasure) getOptional("Dash", 1);
        UnitOfMeasure teaspoonUom = (UnitOfMeasure) getOptional("Teaspoon", 1);
        UnitOfMeasure tablespoonUom = (UnitOfMeasure) getOptional("Tablespoon", 1);
        UnitOfMeasure cupUom = (UnitOfMeasure) getOptional("Cup", 1);
        UnitOfMeasure pinchUom = (UnitOfMeasure) getOptional("Pinch", 1);
        UnitOfMeasure ounceUom = (UnitOfMeasure) getOptional("Ounce", 1);

        Category americanCategory = (Category) getOptional("American", 2);
        Category mexicanCategory = (Category) getOptional("Mexican", 2);
    ...
    private Object getOptional(String description, int i) {
        if (i == 1)
            return unitOfMeasureRepository.findByDescription(description).orElseThrow(() -> new RuntimeException("Expected UOM not found"));
        return categoryRepository.findByDescription(description).orElseThrow(() -> new RuntimeException("Expected Category not found"));
    }
}
